...
// Fetch special pay records with employee name
$stmt = $pdo->query("
...
");
$special_pays = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
.btn-add, .btn-apply {
  font-size: 16px; padding: 10px 20px; border: none; border-radius: 6px; color: white; cursor: pointer; transition: 0.3s;
}
...
<div class="main-content">
  <div class="card">
    <h2>🏅 Special Pay Management</h2>

    <div class="top-buttons">
      <button class="btn-add" data-bs-toggle="modal" data-bs-target="#addSpecialPayModal">
        ➕ Add Special Pay
      </button>
    ...
    <div class="table-responsive">
      <table class="table table-bordered text-center align-middle">
        ...
        <tbody>
          <?php if ($special_pays): foreach ($special_pays as $sp): ?>
            <tr>
              <td><?= htmlspecialchars($sp['special_date']) ?></td>
              <td><?= htmlspecialchars($sp['user_id']) ?></td>
              <td><?= htmlspecialchars($sp['full_name']) ?></td>
              <td>₱<?= number_format($sp['amount'], 2) ?></td>
              <td>
                <span class="status-badge status-<?= $sp['status'] ?>"><?= $sp['status'] ?></span>
              </td>
              <td>
                <a href="delete_special_pay.php?id=<?= $sp['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete this record?')">
                  🗑️ Delete
                </a>
              </td>
            </tr>
...
<!-- Add Special Pay Modal -->
<div class="modal fade" id="addSpecialPayModal" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <form action="add_special_pay.php" method="POST">
        <div class="modal-header bg-primary text-white">
          <h5 class="modal-title">➕ Add Special Pay</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
          <label>Date:</label>
          <input type="date" name="special_date" required class="form-control mb-2">
